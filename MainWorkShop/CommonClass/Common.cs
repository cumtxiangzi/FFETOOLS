    ...
    public class Common
    {
        ...
        static public Curve GetPickSegementWithGrid(UIApplication uiApp, Grid selGrid, XYZ pickPos)
        {
            ...
            if (null != arc)
            {
                ...
                Arc newArc = Arc.Create(arc.Center, arc.Radius, angleStart, angleEnd, XYZ.BasisX, XYZ.BasisY);
                res = newArc;
            }
            Line line = SelGridCurve as Line;
            if (null != line)
            {
                Line newline = Line.CreateBound(intersectPoints[findIndex], intersectPoints[findIndex + 1]);
                res = newline;
            }
            return res;
        }
        ...
        static public List<Curve> GetSegementsWithGrid(UIApplication uiApp, Grid selGrid, List<Grid> allGrids)
        {
            ...
            for (; i < intersectPoints.Count - 1; ++i )
            {
                ...
                if (null != line)
                {
                    Line newline = Line.CreateBound(intersectPoints[i], intersectPoints[i+1]);
                    res.Add(newline);
                }
            }
            return res;
        ...
        static public List<XYZ> UniqueAndSortIntersectPoints(Curve curve, List<XYZ> intersectPoints)
        {
            ...
            if (null != arc)
            {
                ...
                if (normal.Z < 0)
                    ptStart = arc.GetEndPoint(1);
                intersectPoints.Sort(new AngleComparer(ptStart, arc.Center));
            }
            Line line = curve as Line;
            if (null != line)
            {
                XYZ ptStart = line.GetEndPoint(0);
                intersectPoints.Sort(new DistanceComparer(ptStart));
            }

            IEnumerable<XYZ> pts = intersectPoints.Distinct(new Comparint<XYZ>(delegate(XYZ x, XYZ y)
            {
                if (null != x && null != y)
