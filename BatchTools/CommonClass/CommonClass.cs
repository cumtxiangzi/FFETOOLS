...
namespace FFETOOLS
{
    ...
    public static class LineUtil
    {
        ...
        public static double GetPointToLineDistance(XYZ aPoint, Line l) //任意点到线距离
        {
            //先分别得到点的坐标
            XYZ bPoint = l.GetEndPoint(0);
            XYZ cPoint = l.GetEndPoint(1);

            //分别计算a、b、c值
            double a = l.Length;
            double b = aPoint.DistanceTo(cPoint);
            double c = aPoint.DistanceTo(bPoint);
            ...
            //需要判断s值
            if (s < 0 || s == 0) return 0.0; //说明三点不能形成三角形,点在线的延长线上或者在点在线上
            //计算距离
            double ha = 2 * s / a;
            return ha;
        }
    ...
    //机械设备的过滤条件
    public class MechanicalSelectionFilter : ISelectionFilter
    {
        public bool AllowElement(Element e)
        {
            if (e.Category.Name == "机械设备")
            {
                return true;
            }
            else
